        ...
        if p == "h":
            ...
            if len(move) == 4:
                ...
                if problem == False:
                    ...
                    if tMove in vMoves:
                        if (
                            board[int(tMove[0])][int(tMove[1])] == "wp"
                            and int(tMove[3]) == 7
                        ):
                            promote = True
                        elif (
                            board[int(tMove[0])][int(tMove[1])] == "bp"
                            and int(tMove[3]) == 0
                        ):
                            promote = True
                        else:
...
def movePiece(move, take, promote):
    ...
    if promote == False:
        tBoard[int(move[2]) - 1][int(move[3]) - 1] = tBoard[int(move[0]) - 1][
            int(move[1]) - 1
        ]
    while promote == True:
...
def checkForCheck(cBoard, move):
    ...
    for i in range(int(posOfKing[0]) + 1, 8):
        ...
        if (
            tPiece == oppo + "r"
            or tPiece == oppo + "q"
            or (tPiece == oppo + "k" and i - int(posOfKing[0]) == 1)
        ):
            check = True
        elif tPiece != "  ":
            break
    ...
    for i in range(int(posOfKing[1]) + 1, 8):
        ...
        if (
            tPiece == oppo + "r"
            or tPiece == oppo + "q"
            or (tPiece == oppo + "k" and i - int(posOfKing[1]) == 1)
        ):
            check = True
        elif tPiece != "  ":
            break
    ...
    for i in range(1, 8):
        if (
            7 >= int(posOfKing[0]) + i >= 0
            and 7 >= int(posOfKing[1]) + i >= 0
            and cBoard[int(posOfKing[0]) + i][int(posOfKing[1]) + i] != whoTurn + "k"
        ):

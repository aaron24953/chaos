...
class particle:
  ...
  def update(self,w,h,UPS):
    ...
    if self.state==0:
      self.vel[0]*=(1-0.004/UPS)
      self.vel[1]*=(1-0.004/UPS)
    if self.state==1:
      self.vel[0]*=(1+0.006/UPS)
      self.vel[1]*=(1+0.006/UPS)
...
def collide(a,b,uNum):
  ...
  a.vel[0]=(av*math.cos(aa-ca)*(a.mass-b.mass)+2*b.mass*bv*math.cos(ba-ca))/(a.mass+b.mass)*math.cos(ca)+av*math.sin(aa-ca)*math.cos(ca+PI/2)
  a.vel[1]=(av*math.cos(aa-ca)*(a.mass-b.mass)+2*b.mass*bv*math.cos(ba-ca))/(a.mass+b.mass)*math.sin(ca)+av*math.sin(aa-ca)*math.sin(ca+PI/2)
  b.vel[0]=(bv*math.cos(ba-ca)*(b.mass-a.mass)+2*a.mass*av*math.cos(aa-ca))/(b.mass+a.mass)*math.cos(ca)+bv*math.sin(ba-ca)*math.cos(ca+PI/2)
  b.vel[1]=(bv*math.cos(ba-ca)*(b.mass-a.mass)+2*a.mass*av*math.cos(aa-ca))/(b.mass+a.mass)*math.sin(ca)+bv*math.sin(ba-ca)*math.sin(ca+PI/2)
  a.collided=uNum
  b.collided=uNum
